...
yarn_node_modules() {
  ...
  cd "$build_dir" || return
  monitor "yarn-install" yarn install --force --production="$production" --frozen-lockfile --ignore-engines 2>&1
}
...
yarn_prune_devdependencies() {
  ...
  if [ "$NODE_ENV" == "test" ]; then
    ...
    cd "$build_dir" || return
    monitor "yarn-prune" yarn install --force --frozen-lockfile --ignore-engines --ignore-scripts --prefer-offline 2>&1
